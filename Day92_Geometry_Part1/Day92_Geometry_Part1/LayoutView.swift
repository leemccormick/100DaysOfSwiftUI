...
struct LayoutView: View {
    let topic = "How layout works in SwiftUI"
    let info =
    """
    ...
    """
    var body: some View {
        // Color.red --> Fill whole screen : Color.red is a view in its own right, but because it is layout neutral it can be drawn at any size.
        VStack {
            Spacer()
            Text(topic)
                .font(.largeTitle)
                .padding(20)
                .background(.red)
            Text("\(topic) --> The modify order is mater.")
                .background(.red)
                .padding(20)
                .font(.title)
